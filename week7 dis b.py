...
Date: 07/27/2022
Course: ICT-4370-1
Week 7: STOCK PROBLEM WITH OBJECTS AND FILES


'''


#importing datetime library, csv, sys, sqlite3
...
cur.execute('''CREATE TABLE IF NOT EXISTS StockTable
            ([StockName] TEXT,
            [ShareCount] INTEGER,
            [PurchasePrice] REAL,
            [CurrentPrice] REAL,
...
cur.execute('''CREATE TABLE IF NOT EXISTS BondTable
            ([BondName] TEXT,
            [ShareCount] INTEGER,
            [PurchasePrice] REAL,
            [CurrentPrice] REAL,
...
cur.execute('''CREATE TABLE IF NOT EXISTS InvestorTable
...
#cur.execute("INSERT INTO StockTable VALUES ({lines[0]}, {int(lines[1])}, {float(lines[2])}, {float(lines[3]),} {lines[4]}, {dum})")
#cur.execute("INSERT INTO StockTable" "VALUES (%s,%s,%s,%s,%s,%s,)",(1,2,3,4,5,6))
    ...
    with open('D:\Study\MS\Summer 22\Python\Lesson6_Data_Stocks.csv', mode ='r')as file:
        ...
        for lines in list(csvFile)[1:]:
            try:
                ...
                insert_stmt = (
                      "INSERT INTO StockTable (StockName, ShareCount, PurchasePrice, CurrentPrice, PurchaseDate,StockId) "
                      "VALUES(?,?,?,?,?,?)"
                        )
                data = (lines[0], int(lines[1]), float(lines[2]), float(lines[3]), lines[4], dum )
    ...
    with open('D:\Study\MS\Summer 22\Python\Lesson6_Data_Bonds.csv', mode ='r')as file1:
        ...
        for lines in list(csvFile)[1:]:
            try:
                ...
                #inserting data into database
                insert_stmt = (
                      "INSERT INTO BondTable (BondName, ShareCount, PurchasePrice, CurrentPrice, PurchaseDate,BondId, coupon,yield) "
                      "VALUES(?,?,?,?,?,?,?,?)"
                        )
                data = (lines[0], int(lines[1]), float(lines[2]), float(lines[3]), lines[4], dum,float(lines[5]),float(lines[6] ))
                cur.execute(insert_stmt, data)
...
#inserting data into database
try:
    insert_stmt = ("INSERT INTO InvestorTable (investorID, name,address, contactNumber) "
                      "VALUES(?,?,?,?)")                   
    data = (1,"Dheeraj", "S Way St, Aurora, CO", "720-921-9999")
    cur.execute(insert_stmt, data)
except:
...
cur.execute("SELECT * FROM BondTable")
print(cur.fetchall())
cur.execute("SELECT * FROM StockTable")
print(cur.fetchall())
cur.execute("SELECT * FROM InvestorTable")
print(cur.fetchall())
out.close()
connection.close()
